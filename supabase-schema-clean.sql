...
DROP TRIGGER IF EXISTS update_coffee_club_members_updated_at ON coffee_club_members;
DROP FUNCTION IF EXISTS update_updated_at_column();
DROP TABLE IF EXISTS coffee_club_members CASCADE;

-- Create the coffee_club_members table
CREATE TABLE coffee_club_members (
...
CREATE INDEX idx_coffee_club_members_email ON coffee_club_members(email);

-- Create an index on brand for filtering
CREATE INDEX idx_coffee_club_members_brand ON coffee_club_members(brand);
